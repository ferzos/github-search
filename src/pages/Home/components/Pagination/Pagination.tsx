...
const Pagination = (props: Props) => {
  ...
  useEffect(() => {
    ...
    if (Math.abs(totalPage - currentPage) < MAX_PAGE_PAGINATION) {
      for (
        let index = currentPage - MAX_PAGE_PAGINATION;
        index <= currentPage;
        index++
      ) {
        newPages.push(index);
      ...
      for (
        ...
        index < currentPage + clamp(totalPage, MAX_PAGE_PAGINATION);
        index++
      ) {
        newPages.push(index);
      }
    ...
    <div className={style.container}>
      ...
      {pages.map((pageNumber) => (
        <div
          key={pageNumber}
          className={
            currentPage === pageNumber ? style.pageItemActive : style.pageItem
          }
      ...
      {hasNext && (
        <>
          <div className={style.pageItem}>{"..."}</div>
